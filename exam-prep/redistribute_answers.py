...
def shuffle_question_options(question_obj, target_correct_index=None):
    """
    Shuffle the options of a question and update the correct index.
    If target_correct_index is provided, shuffle to make that the correct answer.
    """
    options = question_obj['options']
    current_correct = question_obj['correct']
    
    # Create a list of indices
    indices = list(range(len(options)))
    
    if target_correct_index is not None:
        # Shuffle to place correct answer at target position
        # First, remove the current correct answer
        correct_option = options[current_correct]
        
        # Create new shuffled options
        other_options = [opt for i, opt in enumerate(options) if i != current_correct]
        random.shuffle(other_options)
        
        # Insert correct option at target position
        new_options = other_options[:target_correct_index] + [correct_option] + other_options[target_correct_index:]
        
        question_obj['options'] = new_options
        question_obj['correct'] = target_correct_index
    else:
        # Just shuffle randomly
        combined = list(zip(options, indices))
        random.shuffle(combined)
        new_options, new_indices = zip(*combined)
        
        # Find where the correct answer ended up
        new_correct = new_indices.index(current_correct)
        
        question_obj['options'] = list(new_options)
        question_obj['correct'] = new_correct
    
    return question_obj

def parse_js_questions_file(filepath):
    """Parse a JavaScript questions file and extract questions array."""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find the questions array
    match = re.search(r'const\s+(questions|questionsExtra|questionsExtra2)\s*=\s*\[(.*?)\];', content, re.DOTALL)
    if not match:
        raise ValueError(f"Could not find questions array in {filepath}")
    
    return content, match.group(0), match.group(1)
...
def redistribute_answers(input_file, output_file):
    ...
    # Rebuild the file
    # Replace questions in original content
    new_content = content
...
if __name__ == "__main__":
    files_to_process = [
        ...
        ("questions-extra.js", "questions-extra.js"),
        ("questions-extra2.js", "questions-extra2.js"),
    ]
